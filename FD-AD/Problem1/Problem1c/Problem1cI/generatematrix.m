...
ax(1:N)=h(1:N)./(h(2:N+1).*(h(1:N)+h(2:N+1)));
bx(1:N)=(h(2:N+1)-h(1:N))./(h(1:N).*h(2:N+1));
cx(1:N)=-h(2:N+1)./(h(1:N).*(h(1:N)+h(2:N+1)));

% Creating second der. coeff
     
axx(1:N)=2./(h(2:N+1).*(h(1:N)+h(2:N+1)) );
bxx(1:N)=-2./(h(1:N).*h(2:N+1)) ;
cxx(1:N)=2./(h(1:N).*(h(1:N)+h(2:N+1)));

% Computing matrix

lower=r*Xinner.*cx+0.5*sigma^2*Xinner.^2.*cxx;
mid=r*Xinner.*bx+0.5*sigma^2.*Xinner.^2.*bxx-r;
upper=r*Xinner.*ax+0.5*sigma^2*Xinner.^2.*axx;
