...
function [U]=BSamCallDD_FDA(S0,K,T,r,sigma,D,alpha);
...
% Set up initial condition for next step - coarse grid

S_adapted1=S_adapted1/(1-D);

for j=2:length(S_adapted1)-1
    if soldg_adapted1(j)<max(S_adapted1(j)-K,0)
        p0_save(j)=max(S_adapted1(j)-K,0);
    else
        p0_save(j)=soldg_adapted1(j);
    end    
end
...
% Set up initial condition for next step - adapted grid
