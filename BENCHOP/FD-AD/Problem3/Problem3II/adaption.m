...
function [X_adapted,M]=adaption(sol_at_timestep,timesteps,X_vals,saved_sols,no_of_sols,r,sigmatrix,space_tol,space_fac,max_length)
...
clear Tau 
 ...
 for jj=1:no_of_sols 
    ...
    [A,h_fine]=generatematrix(X_vals,sigmatrix(:,timesteps-ii)',r);
    deltah(:,jj)=A*saved_sols(:,jj); 
    restrVEC=reshape(Rsol_matrix(rind{:}),[prod(vector_lengths_new-2) 1]);
    [A,h2tmp]=generatematrix(X_new,sigmatrix(1:2:end,timesteps-ii)',r);
    delta2h=A*restrVEC;
   ...
   Tau(:,jj)  = (1/3).*(delta2h-deltahRESTR);
